...
Practical significance assessment revealed \textbf{effect sizes well below conventional small effect thresholds} ($|d| = 0.2$), with the largest absolute effect size reaching only 0.065 (Figure~\ref{fig:effect-size-forest}). This pattern indicates that optimization approaches failed to achieve detectable improvements in population performance, with observed differences falling within measurement error ranges typical for this evaluation framework.
